...
BASE_PATH = '/content/drive/MyDrive/project6-2/'
...
def get_dist(start_lat, start_lng, dest_lat, dest_lng):
    client_id = "9miaz3h1lv"
    client_secret = "GZIkCL6SGrTSluUwImMvRyyMw9RZFQZejEPFXMCo"
...
def recommend_nearest_hospitals(user_lat, user_lon, hospital_data, alpha=0.05, top_n=3, max_alpha=5.0, alpha_step=0.05):
    try:
        while alpha <= max_alpha:
            ...
            alpha += alpha_step
        return pd.DataFrame(columns=["hospital_name", "distance_km", "duration", "arrival_time", "address"])
    except Exception as e:
